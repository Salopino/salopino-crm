...
export default function Page() {
  ...
  const [quoteForm, setQuoteForm] = useState({ ...emptyQuote, issue_date: today(), valid_until: addDays(today(), 14) });
  ...
  const [calendarForm, setCalendarForm] = useState({ ...emptyCalendarEvent, start_at: nowLocalInput(), end_at: nowLocalInput() });
  ...
  async function loadData() {
    ...
    try {
      ...
      setQuotes((f.data || []).map((x) => ({ ...x, status: normQuoteStatus(x.status), invoice_status: normInvoiceStatus(x.invoice_status) })));
  ...
  const quotesEnriched = useMemo(
    () =>
      quotes.map((q) => ({
        ...
        lines: quoteLines.filter((l) => l.quote_id === q.id).sort((a, b) => num(a.sort_order) - num(b.sort_order)),
      })),
    [quotes, quoteLines, clientMap]
  );

  const dashboard = useMemo(() => {
    const quotePipeline = quotesEnriched.filter((q) => ["luonnos", "lähetetty"].includes(String(q.status || "").toLowerCase())).reduce((s, q) => s + num(q.total), 0);
    const quoteWon = quotesEnriched.filter((q) => String(q.status || "").toLowerCase() === "hyväksytty").reduce((s, q) => s + num(q.total), 0);
    const quoteLost = quotesEnriched.filter((q) => ["hylätty", "vanhentunut"].includes(String(q.status || "").toLowerCase())).reduce((s, q) => s + num(q.total), 0);
    const cashflowForecast = cashflowEvents.filter((r) => String(r.type || "") === "Ennuste").reduce((s, r) => s + num(r.amount), 0);
    const cashflowRealized = cashflowEvents.filter((r) => r.is_realized === true || ["Tulo", "Meno"].includes(String(r.type || ""))).reduce((s, r) => s + num(r.amount), 0);
    ...
    return { quotePipeline, quoteWon, quoteLost, cashflowForecast, cashflowRealized, monthRevenue, monthTarget, monthTraffic, accountingLatest };
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((c) => {
      const okSearch = !q || [c.name, c.company_name, c.email, c.phone, c.city, c.business_id, c.notes].filter(Boolean).some((v) => String(v).toLowerCase().includes(q));
  ...
  const filteredQuotes = useMemo(() => {
    ...
    return quotesEnriched.filter((x) => {
      const okSearch = !q || [x.quote_number, x.title, x.client?.name, x.client?.company_name, x.invoice_number].filter(Boolean).some((v) => String(v).toLowerCase().includes(q));
  ...
  const financeSummary = useMemo(
    () => ({
      ...
      profit: financeMonthly.reduce((s, r) => s + num(r.profit !== null && r.profit !== undefined ? r.profit : num(r.revenue) - num(r.expenses)), 0),
  ...
  function applyValidityPreset(preset) {
    ...
    setQuoteForm((prev) => ({ ...prev, validityPreset: String(preset), quote_valid_days: days, valid_until: prev.issue_date ? addDays(prev.issue_date, days) : "" }));
  }
  ...
  const updateDraftLine = (id, key, value) => setQuoteDraftLines((prev) => prev.map((l) => (l.id === id ? { ...l, [key]: value } : l)));
  const removeDraftLine = (id) => setQuoteDraftLines((prev) => prev.filter((l) => l.id !== id).map((l, i) => ({ ...l, sort_order: i + 1 })));
  ...
  async function saveClient(e) {
    ...
    try {
      ...
      const q = await supabase.from("clients").insert({ ...clientForm, updated_at: new Date().toISOString(), created_at: new Date().toISOString() });
  ...
  async function saveQuote(e) {
    ...
    try {
      ...
      const lines = quoteDraftLines
        .map((l, i) => ({ description: String(l.description || "").trim(), quantity: num(l.quantity), unit_price: num(l.unit_price), sort_order: i + 1, line_total: num(l.quantity) * num(l.unit_price) }))
      ...
      const ins = await supabase.from("quotes").insert(payload).select().single();
      if (ins.error) throw ins.error;

      const linesIns = await supabase.from("quote_lines").insert(lines.map((l) => ({ ...l, quote_id: ins.data.id, created_at: new Date().toISOString() })));
  ...
  async function createCalendarFromQuote(id) {
    setBusy((b) => ({ ...b, quoteCalendarId: id }));
    try {
      const { error } = await supabase.rpc("create_calendar_event_from_quote", { p_quote_id: id, p_event_type: "Kuvaus" });
      if (error) throw error;
  ...
  async function createCashflowFromQuote(id) {
    setBusy((b) => ({ ...b, quoteCashflowId: id }));
    try {
      const { error } = await supabase.rpc("create_cashflow_from_quote", { p_quote_id: id });
      if (error) throw error;
  ...
  async function saveFinance(e) {
    ...
    try {
      const payload = { ...financeForm, revenue: num(financeForm.revenue), expenses: num(financeForm.expenses), profit: financeForm.profit !== "" ? num(financeForm.profit) : num(financeForm.revenue) - num(financeForm.expenses), target: num(financeForm.target), created_at: new Date().toISOString() };
      const q = await supabase.from("finance_monthly").insert(payload);
      if (q.error) throw q.error;
  ...
  async function saveCashflow(e) {
    ...
    try {
      const q = await supabase.from("cashflow_events").insert({ ...cashflowForm, amount: num(cashflowForm.amount), created_at: new Date().toISOString() });
      if (q.error) throw q.error;
  ...
  async function saveCalendarEvent(e) {
    ...
    try {
      ...
      const q = await supabase.from("calendar_events").insert({ ...calendarForm, start_at: new Date(calendarForm.start_at).toISOString(), end_at: new Date(calendarForm.end_at).toISOString(), amount_estimate: num(calendarForm.amount_estimate), created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
      if (q.error) throw q.error;
  ...
  async function saveTemplate(e) {
    ...
    try {
      const payload = { ...templateForm, name: templateForm.name || templateForm.service_name, service_name: templateForm.service_name || templateForm.name, unit_price: num(templateForm.unit_price), vat_rate: num(templateForm.vat_rate), created_at: new Date().toISOString() };
      const q = await supabase.from("pricing_templates").insert(payload);
      if (q.error) throw q.error;
  ...
  async function saveAccountingDocument(e) {
    ...
    try {
      const q = await supabase.from("accounting_documents").insert({ ...accountingDocumentForm, uploaded_at: new Date().toISOString(), created_at: new Date().toISOString() });
      if (q.error) throw q.error;
      await supabase.from("import_logs").insert({ import_type: accountingDocumentForm.document_type, source_name: accountingDocumentForm.file_url || accountingDocumentForm.file_name || "Kirjanpidon aineisto", status: "Tallennettu", row_count: 1, message: `Kirjanpidon aineisto tallennettu (${accountingDocumentForm.source_system})`, source_system: accountingDocumentForm.source_system, file_name: accountingDocumentForm.file_name || null, month: accountingDocumentForm.month || null, imported_at: new Date().toISOString() });
  ...
  async function saveAccountingMonthly(e) {
    ...
    try {
      const payload = {
        ...accountingMonthlyForm,
        revenue: num(accountingMonthlyForm.revenue),
        other_income: num(accountingMonthlyForm.other_income),
        materials_and_services: num(accountingMonthlyForm.materials_and_services),
        personnel_expenses: num(accountingMonthlyForm.personnel_expenses),
        other_operating_expenses: num(accountingMonthlyForm.other_operating_expenses),
        depreciation: num(accountingMonthlyForm.depreciation),
        operating_profit: num(accountingMonthlyForm.operating_profit),
        financial_items: num(accountingMonthlyForm.financial_items),
        profit_before_appropriations: num(accountingMonthlyForm.profit_before_appropriations),
        balance_assets: num(accountingMonthlyForm.balance_assets),
        balance_liabilities: num(accountingMonthlyForm.balance_liabilities),
        equity: num(accountingMonthlyForm.equity),
        cash_and_bank: num(accountingMonthlyForm.cash_and_bank),
        receivables: num(accountingMonthlyForm.receivables),
        payables: num(accountingMonthlyForm.payables),
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      };
  ...
  const nav = [["dashboard", "Dashboard"], ["crm", "CRM"], ["quotes", "Tarjoukset"], ["finance", "Talous"], ["calendar", "Kalenteri"], ["settings", "Asetukset"]];
      ...
      <div style={sx.wrap}>
        <header style={{ display: "grid", gridTemplateColumns: "1.15fr .85fr", gap: 18, marginBottom: 22 }}>
          <div style={sx.hero}>
            ...
            <h1 style={{ margin: "0 0 10px", fontSize: 42, lineHeight: 1.05, fontWeight: 900 }}>Tarjous → kalenteri → kassavirta → kirjanpito</h1>
            <p style={{ margin: 0, color: "rgba(244,241,233,.74)", fontSize: 16, lineHeight: 1.7 }}>
              Yksi yhtenäinen buildaava versio. Tarjouksiin on lisätty voimassaolo 7/10/14/21/30/oma, kalenteri- ja kassavirtaennusteet sekä Netvisor-linkkien ja kuukausitarkennusten tallennus.
            </p>
          </div>
          <div style={sx.card}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10 }}>
              {nav.map(([k, label]) => (
                <button key={k} onClick={() => setView(k)} style={{ padding: "14px 12px", borderRadius: 14, color: "#f4f1e9", fontWeight: 700, cursor: "pointer", border: view === k ? "1px solid rgba(231,223,178,.34)" : "1px solid rgba(231,223,178,.10)", background: view === k ? "linear-gradient(135deg, rgba(217,201,138,.22), rgba(120,92,35,.18))" : "rgba(10,10,16,.84)" }}>
                  {label}
                </button>
              ))}
            </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18 }}>
                  ...
                  <div style={sx.card}>
                    ...
                    <div style={{ lineHeight: 1.8, color: "rgba(244,241,233,.74)" }}>
                      Tarjoukselta voi luoda kuvausvarauksen kalenteriin sekä kassavirtaennusteen. Talous-näkymään voi tallentaa Netvisorista tuotujen raporttien linkit ja kuukausitarkennukset.
                    </div>
                  </div>
                </div>
              </section>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "560px 1fr", gap: 18 }}>
                  <div style={sx.cardDark}>
                    <form onSubmit={saveQuote}>
                      <div style={{ display: "grid", gap: 12 }}>
                        <Field label="Asiakas"><Select value={quoteForm.client_id} onChange={(e) => setQuoteForm({ ...quoteForm, client_id: e.target.value })}><option value="">Valitse asiakas</option>{clients.map((c) => <option key={c.id} value={c.id}>{c.name}{c.company_name ? ` – ${c.company_name}` : ""}</option>)}</Select></Field>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                          <Field label="Voimassaolo"><Select value={quoteForm.validityPreset} onChange={(e) => applyValidityPreset(e.target.value)}>{VALIDITY_PRESETS.map((v) => <option key={v} value={v}>{v === "oma" ? "oma päivä" : `${v} pv`}</option>)}</Select></Field>
                        ...
                        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
                          <label style={{ display: "flex", gap: 8, alignItems: "center" }}><input type="checkbox" checked={!!quoteForm.auto_create_calendar} onChange={(e) => setQuoteForm({ ...quoteForm, auto_create_calendar: e.target.checked })} /><span>Luo kalenteri tarjoukselta</span></label>
                          <label style={{ display: "flex", gap: 8, alignItems: "center" }}><input type="checkbox" checked={!!quoteForm.auto_create_cashflow} onChange={(e) => setQuoteForm({ ...quoteForm, auto_create_cashflow: e.target.checked })} /><span>Luo kassavirtaennuste tarjoukselta</span></label>
                        </div>
                        <div style={{ padding: 14, borderRadius: 16, background: "rgba(10,10,16,.58)", border: "1px solid rgba(231,223,178,.12)" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><strong>Tarjousrivit</strong><Btn type="button" variant="ghost" onClick={addDraftLine}>Lisää rivi</Btn></div>
                          <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={sx.card}>
                    ...
                    <table style={sx.table}>
                      <thead><tr>{["Nro", "Otsikko", "Status", "Kuvauspäivä", "Laskutus", "Summa", "Toiminnot"].map((h) => <th key={h} style={sx.th}>{h}</th>)}</tr></thead>
                      <tbody>
                        {filteredQuotes.map((q) => (
                          <tr key={q.id}>
                            ...
                            <td style={sx.td}><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Btn variant="ghost" onClick={() => invoiceQuote(q.id)}>{busy.invoicingQuoteId === q.id ? "..." : "Laskuta"}</Btn><Btn variant="ghost" onClick={() => createCalendarFromQuote(q.id)}>{busy.quoteCalendarId === q.id ? "..." : "Kalenteri"}</Btn><Btn variant="ghost" onClick={() => createCashflowFromQuote(q.id)}>{busy.quoteCashflowId === q.id ? "..." : "Kassavirta"}</Btn></div></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div style={sx.card}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                      <div>
                        ...
                        <table style={sx.table}>
                          <thead><tr>{["Kuukausi", "Tyyppi", "Lähde", "Tiedosto / linkki"].map((h) => <th key={h} style={sx.th}>{h}</th>)}</tr></thead>
                          <tbody>
                            {accountingDocuments.map((r) => (
                              <tr key={r.id}>
                                ...
                                <td style={sx.td}>{r.file_url ? <a href={r.file_url} target="_blank" rel="noreferrer" style={{ color: "#d9c98a" }}>{r.file_name || r.file_url}</a> : r.file_name || "-"}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      <div>
                        ...
                        <table style={sx.table}>
                          <thead><tr>{["Kuukausi", "Liikevaihto", "Liiketulos", "Pankki", "Oma pääoma"].map((h) => <th key={h} style={sx.th}>{h}</th>)}</tr></thead>
                          <tbody>
                  ...
                  <div style={sx.card}>
                    ...
                    <div style={{ marginTop: 22 }}>
                      ...
                      <table style={sx.table}>
                        ...
                        <tbody>
                          {importLogs.filter((r) => !importFilter ? true : [r.import_type, r.source_name, r.status, r.file_name, r.source_system].filter(Boolean).some((v) => String(v).toLowerCase().includes(importFilter.toLowerCase()))).map((r) => (
                            <tr key={r.id}>
